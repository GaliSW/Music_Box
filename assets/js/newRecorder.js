class Recorder {
     ...
     */
    getWav() {
        if (this._mergedChannelData == null) this._mergeChannels();

        let wav = new Wav(
            this._mergedChannelData,
            this._actualChannelCount,
    ...
    _wireRecordingStream(stream) {
        ...
        this._node = (
            this._context.createScriptProcessor ||
            this._context.createJavaScriptNode
        ).call(
            this._context,
            4096,
            this._desiredChannels,
            this._desiredChannels
        ); // 4K buffer and we prefer a single (mono) channel.
    ...
    _mergeChannels() {
        if (this._actualChannelCount === 2) {
            this._mergedChannelData = this._interleave(
                this._channels[0],
                this._channels[1]
            );
        } else {
...
class Channel {
     ...
     */
    getAudioData() {
        if (this._flattened == null) this._flatten();
