var app = new Vue({
    ...
    watch: {
        ...
        hint: async function (val, oldVal) {
            if (val === "") return;
            ...
            function disable() {
                return new Promise((resolve, reject) => {
                    setTimeout(() => {
                        ...
                        document.querySelector(".hint").style.visibility =
                            "hidden";
                        resolve();
                    }, 1500);
                });
    ...
    methods: {
        ...
        tutorMark() {
            ...
            if (this.tutor) {
                for (let i = 0; i < this.tutorStart.length; i++) {
                    ...
                    // if (i === app.tutorStart.length - 1) {
                    //     app.tutorLastIndex = app.tutorStart[i];
                    // }
                }
            } else {
                for (let i = 0; i < this.tutorStart.length; i++) {
                ...
                .then((res) => {
                    ...
                    function onPlayerStateChange3(e) {
                        ...
                        if (e.data == 1) {
                            app.tutorstate = 1;
                            // app.timer = setInterval(app.fnTimeChecking, 10);
                            function fnPlayer3TimeCheck(timestamp) {
                                if (
                                    !app.lastTime ||
                                    timestamp - app.lastTime >= 100
                                ) {
                                    app.player3CurrentTime = player3
                                        .getCurrentTime()
                                        .toFixed(1);
                                    app.lastTime = timestamp;
                                }
                                app.player3TimeCheck =
                                    requestAnimationFrame(fnPlayer3TimeCheck);
                            }
                            app.player3TimeCheck =
                                requestAnimationFrame(fnPlayer3TimeCheck);
                        }
                        if (e.data == 2) {
                            app.tutorstate = 0;
                            cancelAnimationFrame(app.player3TimeCheck);
                            // clearInterval(app.timer);
                        }
                    }
            ...
            if (this.tutor && this.nowPlaying >= 1) {
                ...
                if (
                    ...
                    (app.player3CurrentTime == tutorEndTime &&
                        Number(app.player3CurrentTime) > 0)
                ) {
                    if (!app.tutorIsEnd) {
                        if (window.innerWidth > 991) {
                            player2.pauseVideo();
                        ...
                        // if (app.nowPlaying - 1 === app.tutorLastIndex) {
                        ...
                        if (app.singleMode) return; //單句模式不滾動
                        // 提前觸發
                        listWindowContent.scrollTo({
                            top: sutitleBlkTop, //包含上下行距
                        });
                        app.tutorPlaying = false;
                        app.tutorIsEnd = true;
                    }
                }
            }
        ...
        tutorPlay(state) {
            ...
            if (state == 1) {
                app.timer = setInterval(app.fnTimeChecking, 100);
                if (window.innerWidth > 991) {
                    player2.playVideo();
                } else {
                    player3.playVideo();
        ...
        fnSeekTo(event) {
            ...
            if (tutorArr.slice(-1)[0] == undefined) {
                app.lastTutorTime = 0;
            } else {
        ...
        async tabChange(evt) {
            ...
            if (this.nowtab == 0) {
                console.log(app.audioTime);
                for (let i = 0; i < app.audioTime.length; i++) {
                ...
                if (this.firstTab && this.mobileType == "") {
                    ...
                    if (
                        document.querySelector(
                            `.audioLength${this.nowPlayAudioIndex}`
                        )
                    ) {
                        document
                            .querySelector(
                        ...
                        document
                            .querySelector(
                                `.audioTimer${this.nowPlayAudioIndex}`
                            )
                            .classList.add("none");

                        document
                            .getElementById(
                                `audioPlay${this.nowPlayAudioIndex}`
                            )
                            .classList.remove("click");
        ...
        getAudioTime() {
            return new Promise((resolve, reject) => {
                let timeArr = [];
                for (let i = 0; i < app.likeData.length; i++) {
                    let duration;
            ...
            return new Promise((resolve, reject) => {
                axios
                    ...
                    .then(async (res) => {
                        ...
                        this.likeData = await res.data[Object.keys(res.data)];
                        setTimeout(async () => {
                            await this.getAudioTime();
                            resolve();
                        }, 1500);
        ...
        recStop() {
            ...
            app.rec.stop(function (blob, duration) {
                ...
                // console.log(app.recBlob[0], audio.src);
                // setTimeout(function () {
                //     (window.URL || webkitURL).revokeObjectURL(audio.src);
                // }, 1000);
                // app.recClose();
        ...
        setContext(id) {
            if (app.audioContextId.indexOf(id) > -1) {
                return;
            } else {
                app.audioContextId.push(id);
                return new Promise((resolve, reject) => {
                    if (app.nowPlayAudioIndex == id) {
                        return resolve();
                    } else {
                        app.audioContext = app.fnReverb(
    ...
    async created() {
        ...
        document.querySelector(".loading_blk").classList.add("none");
